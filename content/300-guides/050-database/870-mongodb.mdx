---
title: 'Using Prisma with Mongo Database'
metaTitle: 'Using Prisma with Mongo Database'
...
Some aspects of using Prisma with Mongo Databaseare the same as when using Prisma with a relational database. You can still:
...
MongoDB's document-based structure and flexible schemas means that using Prisma with Mongo Databasediffers from using it with a relational database in a number of ways. These are some areas where there are differences that you need to be aware of:
...
## How to use Prisma with Mongo Database
